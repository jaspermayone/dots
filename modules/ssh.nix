...
in
{
  options.jsp.ssh = {
    ...
    zmx = {
      ...
      hosts = mkOption {
        ...
        default = [ ];
        description = "List of host patterns to enable zmx auto-attach (e.g., 'pve.*')";
      };
    };
        ...
        types.submodule {
          options = {
            ...
            addKeysToAgent = mkOption {
              type = types.nullOr types.str;
              default = null;
              description = "Add keys to SSH agent (yes/no/confirm/ask)";
            };
  ...
  config = mkIf cfg.enable {
    # Install zmx and autossh when zmx is enabled
    home.packages = optionals cfg.zmx.enable [
      pkgs.zmx-binary
      pkgs.autossh
    ];
    ...
    # Add shell aliases for zmx usage
